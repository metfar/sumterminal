...
def show_preferences(preferences=None):
    ...
    current=window("SUM Terminal Preferences",width=760,height=930,base_width=760,base_height=930,theme=gui_theme(value.general.theme),font_name=value.general.font_name,font_size=18);
    ...
    font_picker=current.add(FontPicker(current.rect(270,264,440,84),current.font,family=value.general.font_name,bold=value.general.font_bold,italic=value.general.font_italic,small_caps=value.general.font_small_caps,theme=current.theme,monospace_only=True,max_rows=9));
    label("Font size",28,370,200,34); font_size=slider("{} pt".format(value.general.font_size),270,362,440,54,minimum=8,maximum=48,value=value.general.font_size,step=1);
    label("Drop-down",28,430,650,34,font_size=20,bold=True);
    label("Shortcut",28,476,230,34); shortcut=current.add(ShortcutInput(current.rect(270,470,440,42),current.font,text=value.dropdown.shortcut,placeholder="Click, then press shortcut",max_length=48,theme=current.theme));
    label("Height",28,532,190,34); height=slider("{}%".format(value.dropdown.height),270,524,440,54,minimum=10,maximum=100,value=value.dropdown.height,step=1);
    label("Width",28,592,190,34); width=slider("{}%".format(value.dropdown.width),270,584,440,54,minimum=20,maximum=100,value=value.dropdown.width,step=1);
    label("Opacity",28,652,190,34); opacity=slider("{}%".format(int(round(value.dropdown.opacity*100))),270,644,440,54,minimum=20,maximum=100,value=value.dropdown.opacity*100,step=1);
    status=label("Fonts are alphabetical; type any substring to filter. Ctrl+F12 toggles; Ctrl+Shift+T opens a tab.",28,714,684,48,font_size=15);
    ...
    button("APPLY",310,846,180,54,do=lambda:save(False)); button("SAVE && CLOSE",510,846,200,54,do=lambda:save(True));
    start(); return 0;
